...
use crate::{
    chunk::{ChunkData, ChunkEntityData, ChunkReadingError, ChunkWritingError},
    level::{LevelFolder, SyncChunk, SyncEntityChunk},
};
...
pub struct EntityChunkFileManager<S: ChunkSerializer<WriteBackend = PathBuf>> {
    // Dashmap has rw-locks on shards, but we want per-serializer.
    //
    // Lock lookups need to be:
    // - Relatively quick insertion / lookup as to not extraneously block other concurrent hashmap
...
impl<S: ChunkSerializer<WriteBackend = PathBuf>> Default for EntityChunkFileManager<S> {
    fn default() -> Self {
        Self {
...
impl<S: ChunkSerializer<WriteBackend = PathBuf>> EntityChunkFileManager<S> {
...
impl<S: ChunkSerializer<Data = ChunkEntityData, WriteBackend = PathBuf>> EntityChunkFileManager<S> {
...
#[async_trait]
impl<S> FileIO for EntityChunkFileManager<S>
where
    S: ChunkSerializer<Data = ChunkEntityData, WriteBackend = PathBuf>,
{
    ...
    async fn fetch_chunks(
        ...
        folder: &LevelFolder,
        chunk_coords: &[Vector2<i32>],
        stream: tokio::sync::mpsc::Sender<LoadedData<SyncEntityChunk, ChunkReadingError>>,
    ) {
        let mut regions_chunks: BTreeMap<String, Vec<Vector2<i32>>> = BTreeMap::new();
        ...
        let region_read_tasks = regions_chunks.into_iter().map(async |(file_name, chunks)| {
            ...
            // Intermediate channel for wrapping the data with the Arc<RwLock>
            let (send, mut recv) =
                mpsc::channel::<LoadedData<ChunkEntityData, ChunkReadingError>>(1);
    ...
    async fn save_chunks(
        &self,
        folder: &LevelFolder,
        chunks_data: Vec<(Vector2<i32>, SyncEntityChunk)>,
    ) -> Result<(), ChunkWritingError> {
        let mut regions_chunks: BTreeMap<String, Vec<SyncEntityChunk>> = BTreeMap::new();
